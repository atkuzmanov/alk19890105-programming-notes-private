...
function showFlatCode(dataMap, count) {
  ...
  for (var key in dataMap) {
    ...
    if (code) {
      data += "<div class=\"b-c-s\">" + fixString(code) + "</div>";
    }
  }
...
function processRecentPart(recentDivId, link, listClass, excessHeight, bLink) {
    $(document).ready(function () {
        setTimeout(function () {
           ...
           let ebayDiv =$("div.right-wrapper #auction-nudge-items");
           ...
           var siteAdDiv = $("#" + recentDivId);
            ...
            addRecentContent(recentDivId, h - offsetTop - 100 - excessHeight, link, listClass, bLink);
        }, 5000);
    });
}
...
function addRecentLink(recentDivId, maxAllowedHeight, bLink, listClass) {
    ...
    var baseStr = base.protocol + "//" + base.host + "/" + bl;

    setTimeout(() => addGroup(baseStr, loadedRecent, 0, targetElement, maxAllowedHeight, listClass));
...
function addLine(baseStr, groupArray, groupCounter, targetElement, counter, maxAllowedHeight, list, listClass) {
    ...
    if (targetElement.offsetHeight + 50 > maxAllowedHeight) {
        //  console.log('returning ' + targetElement.offsetHeight + " -- " + maxAllowedHeight);
        return;
    }
    if (counter >= list.length) {
        setTimeout(() => addGroup(baseStr, groupArray, groupCounter + 1, targetElement, maxAllowedHeight, listClass));
        return;
    }
